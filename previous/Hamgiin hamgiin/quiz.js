...
function checkAnswer(selectedChoice, selectedButton) {
  ...
  if (selectedChoice === currentQuestion.answer) {
    score++;
    feedbackContainer.textContent = `Дұрыс!`;
  } else {
    feedbackContainer.textContent = `Дұрыс емес. Дұрыс жауап: ${currentQuestion.answer}`;
  }
...
function showResults() {
  ...
  a = `Жарайсың жаным. Ұпайыңыз: ${score} / ${questions.length}.`
  }else
  {
    ...
    a = `Жаман емес жаным.  Ұпайыңыз: ${score} / ${questions.length}.`
  }
  quizContainer.innerHTML = `
     <img src="${photo}" alt="Result Image" style="max-width: 150px; margin: 20px 0;">
    <h2>Тест аяқталды!</h2>
    <p>${a}</p>
    <button onclick="restartQuiz()">Тестті қайта бастау</button>
    <button onclick="selectQuiz()">Басқа тақырыпты таңдау</button>
  `;
}
