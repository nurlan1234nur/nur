...
//let word = words[Math.floor(Math.random() * words.length)].hint;
...
function showHint() {
    const akk = word[0];
    hint.textContent = `Hint: First letter: ${akk.toUpperCase()}`;
}

function resetGame() {
    livecount = 0;
    inWord = Array(word.length).fill("_");
    ...
    button.classList.remove("correct", "uncorrect");});
    showHint();  
}
